...
#本地端開發聊天機器人要確定的事情
#1.應用程式有正常啟動(至少要保留一個終端機去跑python程式)
#啟動應用程式
# python3 app.py
# 按下ctrl+c結束程式

#2.ngrok必須要打開有正常啟動(免費版的ngrok每次的網址都是隨機的，要隨機貼一次網址到webhook)
# ./ngrok http 5001 (要對應到最後面port的對應值數字

#3.ngrok的網址要跟line機器人的webhook網址一樣
...
client = OpenAI(api_key=os.environ.getenv("OPENAI_API_KEY")) #要付錢之後才有api_key
...
def handle_message(event): #event是一個物件
    with ApiClient(configuration) as api_client:
        ...
        print("#" * 30)
        bot_msg = TextMessage(text=f"你幹嘛跟我說{user_msg}")
        bot_msg = menu
        #如果user說的話在faq裡面作為key
        #將value作為機器人的回應
        if user_msg in faq:
            ...
            report = f"{user_msg}的匯率是:\n買價:{buy}\n賣價:{sell}\n資料來源:臺灣銀行匯率公告，正式價格以臺灣銀行網站公告為準。"
            ...
            completion = client.chat.completions.create(
                ...
                messages=[
                    #System prompt
                    {"role": "system", "content": "你是一個很會講諧音梗笑話的機器人，你可以跟使用者聊天逗他們笑"},
                    #User prompt
    ...
    with ApiClient(configuration) as api_client:
        ...
        line_bot_api.reply_message_with_http_info(
            ReplyMessageRequest(
                reply_token=event.reply_token,
                messages=[
                    StickerMessage(package_id="6632", sticker_id="11825393"),
